...
| `TraceSpanTree` span hierarchy renderer | `apps/console/src/components/explorer/TraceSpanTree.tsx` | ✅ Done |
| `EventsExplorer` generic event list | `apps/console/src/components/explorer/EventsExplorer.tsx` | ✅ Done |
...
> Goal: ship a working AI Events tab, workflow waterfall, and token/cost widgets.
...
- "New Dataset" button
- Per-dataset view: item list, "Run Eval" button (triggers an eval run against a selected prompt version)
...
No new backend needed — `retrieval` events are already stored.
...
- `metric=top-score-distribution` — histogram of `payload.top_score` (vector/hybrid only)
...
Phase 1 (3–4 days)
  ...
  1.5  TraceSpanTree AI enhancements
  1.6  Token/cost widget types + /api/events/ai/stats route
  1.7  Console MDX docs

Phase 2 (8–12 days)
  2.1  Prompt library (DB + API + UI)
  2.2  Cost alerting (worker + rule type)
  2.3  Safety dashboard (API + UI)
  2.4  Quality scoring display (UI only)
  2.5  Eval datasets (DB + sampling worker + regression worker + UI)
  2.6  Human feedback (UI button + API + widget)
  2.7  RAG widgets (stats API additions + widget registry)

Phase 3 (2–3 weeks)
  3.1  AI audit trail UI + export
  3.2  AgentAuth linkage (filter + agent detail page + IAM endpoint)
  3.3  Multi-agent waterfall UI
  3.4  Anomaly detection worker
```
